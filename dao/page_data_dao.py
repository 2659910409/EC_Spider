...
class PageDao:
    def insert(self, website, menulevel_first, url, menulevel_second=None, menulevel_third=None):
        key = DB.insert("insert into t_page (website, menulevel_first, menulevel_second, menulevel_third, url, created, updated) values({1}, {2}, {3}, {4}, {5}, {6}, {7})".format(website, menulevel_first, menulevel_second, menulevel_third, url, get_current_timestamp(), get_current_timestamp()))
        return key
...
class PageDataConfDao:
    def insert(self, page_data_id, p_type, p_key, p_value, p_description):
        key = DB.insert("insert into t_page_data_conf (page_data_id, p_type, p_key, p_value, p_description, created, updated) values({1}, {2}, {3}, {4}, {5}, {6}, {7})".format(page_data_id, p_type, p_key, p_value, p_description, get_current_timestamp(), get_current_timestamp()))
        return key
...
class PageDataDao:
    def insert(self, page_id, data_name, data_source_type, data_update_freq, data_update_time):
        key = DB.insert("insert into t_page (page_id, data_name, data_source_type, data_update_freq, data_update_time, created, updated) values({1}, {2}, {3}, {4}, {5}, {6}, {7})".format(page_id, data_name, data_source_type, data_update_freq, data_update_time, get_current_timestamp(), get_current_timestamp()))
        return key
...
class DataTabDao:
    def insert(self, name, page_data_id, check_name_rule, business_columns, pre_cnt=1):
        key = DB.insert("insert into t_data_tab (name, page_data_id, check_name_rule, business_columns, pre_cnt, created, updated) values({1}, {2}, {3}, {4}, {5}, {6}, {7})".format(name, page_data_id, check_name_rule, business_columns, pre_cnt, get_current_timestamp(), get_current_timestamp()))
        return key
    ...
    def query(self, id):
        data = DB.query_fetchall('select id, name, page_data_id, check_name_rule, business_columns, pre_cnt, created, updated from t_data_tab where id = {}'.format(id))
        return data
